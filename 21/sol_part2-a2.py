...
def preprocess():
    ...
    actionState = [(2, 1),] * 26
    upState = [(2, 1),] * 25 + [(1, 1),]
    downState = [(2, 1),] * 25 + [(1, 0),]
    leftState = [(2, 1),] * 25 + [(0, 0),]
    rightState = [(2, 1),] * 25 + [(2, 0),]
    AtU = cost_between_states(RobotChain(copy.copy(actionState)), tuple(upState))
    AtD = cost_between_states(RobotChain(copy.copy(actionState)), tuple(downState))
    AtR = cost_between_states(RobotChain(copy.copy(actionState)), tuple(rightState))
    AtL = cost_between_states(RobotChain(copy.copy(actionState)), tuple(leftState))
    print((AtU, AtD, AtR, AtL))
